...
export class BookingService {
  ...
  private bookingList: Booking[] = [new Booking(1, "Juan Pérez", "+34 600 123 456", "juan.perez@example.com", 4, "Prefiere mesa cerca de la ventana.", new
  ...
  deleteBooking(id: number) {
    let index = this.bookingList.findIndex((booking) => booking.id === id);
    this.bookingList.splice(index, 1);
  }

  getBookingById(id: number): Booking | null {
    for (let i = 0; i < this.bookingList.length; i++) {
      if (id == this.bookingList[i].id) {
        return this.bookingList[i]
      }
    }
    return null;
  }
  ...
  addBooking(booking: Booking) {
    const index = this.bookingList.findIndex(bookingList => booking.id === bookingList.id);
    console.log(index)
    if (index <= -1) {
      booking.id = Math.trunc(Math.random() * 1000) + 1;
